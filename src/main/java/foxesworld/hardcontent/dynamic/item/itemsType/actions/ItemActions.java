...
package foxesworld.hardcontent.dynamic.item.itemsType.actions;
...
public class ItemActions {
    ...
    private void xplode(float itemRequestData) {
        xplode = new Thread(new Runnable() {
            public void run() {
                //final boolean b = world.spawnEntity(new EntityAIWander(new EntityVindicator(world), itemRequestData));
                //world.spawnEntity(new EntityTNTPrimed(world, pMethods.getPlayerLook('x'), pMethods.getPlayerLook('y'), pMethods.getPlayerLook('z'), player));//player, pMethods.getPlayerLook('x'), pMethods.getPlayerLook('y'), pMethods.getPlayerLook('z'), itemRequestData, true);
            }
        });
    }
}

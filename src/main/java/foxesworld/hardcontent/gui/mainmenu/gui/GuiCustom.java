/*
 * Copyright (c) 2022  GuiCustom by FoxesWorld
 *
...
import foxesworld.hardcontent.gui.mainmenu.lib.MODE;
...
public class GuiCustom extends GuiScreen implements GuiYesNoCallback {
    ...
    public void initGui() {
        ...
        if (calendar.get(2) + 1 == 10 && calendar.get(5) == 17) {
            this.splashText = "Happy birthday, AidenFox";
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (Image i : guiConfig.customImages) {
            ...
            int width = i.width;
            int height = i.height;
            //if(width==0){width=mc.displayWidth;}
            //if(height==0){height=mc.displayHeight;}

            if (i.ichBinEineSlideshow) {
                i.slideShow.getCurrentResource1().bind();
                RenderUtil.drawCompleteImage(posX, posY, width, height);
                ...
                if (i.hoverImage != null && mouseX >= posX && mouseX <= posX + width && mouseY >= posY && mouseY <= posY + height) {
                    i.hoverImage.bind();
                } else {
                    i.image.bind();
                }
